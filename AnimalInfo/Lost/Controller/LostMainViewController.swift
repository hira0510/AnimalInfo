...
class LostMainViewController: AnimalInfoBaseViewController {
    
    @IBOutlet var views: LostMainViews!
    private let viewModel = LostMainViewModel()
    
    override func viewDidLoad() {
        super.viewDidLoad()
        setupCollection()
        requestLostDataToObject()
    }
    ...
    private func requestLostDataToObject() {
        viewModel.requestLostData().subscribe(onNext: { [weak self] (result) in
...
extension LostMainViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case .banner:
            ...
            let model = viewModel.filterData()
            cell.configLost(vc: self, banners: model)
            return cell
        case .filter:
        ...
        case .info:
            ...
            let model = viewModel.filterData()
            cell.configAdoptCell(adopt: nil, lost: model[indexPath.item])
            return cell
        }
    }
...
extension LostMainViewController: AnimalInfoSearchCellProtocol {
...
// MARK: - AnimalInfoSearchCellProtocol
extension LostMainViewController: AnimalInfoFilterMainCellProtocol {
